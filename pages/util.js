...
const util = {
  ...
  maskInput() {
    window
      .Inputmask("datetime", { inputFormat: "mm/dd/yyyy" })
      .mask("input[data-type=date]");
    window
      .Inputmask("datetime", { inputFormat: "HH:MM:ss" })
      .mask("input[data-type=time]");
    window
      .Inputmask("datetime", { inputFormat: "mm/dd/yyyy HH:MM:ss" })
      .mask("input[data-type=datetime]");
  ...
  setSearchParams() {
    if (location.pathname.toLowerCase().endsWith("create")) {
      new URLSearchParams(location.search).forEach((value, key) => {
        let element =
          document.getElementById(key) || document.getElementById(key + value);
        if (element) {
  ...
  searchChange() {
    ...
    let type =
      document
        .getElementById("search_col")
        .selectedOptions[0].getAttribute("data-type") || "text";
  ...
  validateForm() {
    let password = document.querySelector(
      "input[type=password]:not([data-match])",
    );
    ...
    if (match) {
      isPasswordMatch =
        document.getElementById(match.getAttribute("data-match")).value ==
        match.value;
    }
    if (passwordError) {
      alert(passwordError);
  ...
  getLink(paging, type, value, sort = "") {
    ...
    if (type == "sort") {
      link =
        ...
        "&sort=" +
        value +
        ((query.sort == value || (!query.sort && sort == "asc")) && !query.desc
          ? "&desc=1"
          : "");
    } else if (type == "page") {
      link =
        ...
        paging.size +
        (query.sort
          ? "&sort=" + query.sort + (query.desc ? "&desc=1" : "")
          : "");
    } else if (type == "size") {
      link =
        "?page=1&size=" +
    ...
    link += query.sw
      ? "&sw=" + query.sw + "&sc=" + query.sc + "&so=" + query.so
      : "";
    return link;
  },
